...
from lunar_fno.data.waveform_dataset import WaveformDataset
from lunar_fno.models.fno1d import SimpleBlock1d
...
def run(config: dict):
    ...
    for seed in seed_list:
        ...
        train_loader = DataLoader(
            WaveformDataset(x_train, y_train),
        ...
        val_loader = DataLoader(
            WaveformDataset(x_val, y_val),
            batch_size=int(train_cfg["batch_size"]),
            shuffle=False,
            num_workers=int(runtime_cfg.get("num_workers", 0)),
        )
        test_loader = DataLoader(
            WaveformDataset(x_test, y_test),
            batch_size=int(train_cfg["batch_size"]),
            shuffle=False,
            num_workers=int(runtime_cfg.get("num_workers", 0)),
        )
        mq_loader = DataLoader(
            WaveformDataset(x_mq, y_mq),
            batch_size=int(train_cfg["batch_size"]),
            shuffle=False,
            num_workers=int(runtime_cfg.get("num_workers", 0)),
        )
        ...
        early_stop_rows.append({"modality": "waveform", "seed": seed, "early_stop_epoch": stop_epoch})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", required=True, help="Path to waveform config YAML")
